...
module PgPartitioned
  class AdapterDecorator < SimpleDelegator
    ...
    def create_partition_schema(table_name)
      schema_name = partition_schema_name(table_name)
      create_schema(schema_name) unless schema_exists?(schema_name)
    end

    def drop_partition_schema(table_name)
      schema_name = partition_schema_name(table_name)
    ...
    def create_partition_of(table_name, constraint_clause, **options)
      child_table_name    = partition_schema_table_name(table_name, options[:prefix]) if options[:prefix]
    ...
    def partition_schema_table_name(table_name, prefix)
      [partition_schema_name(table_name), "#{table_name}_#{prefix}"].join('.')
    end

    def partition_schema_name(table_name)
      "#{table_name}_partitions"
    end
